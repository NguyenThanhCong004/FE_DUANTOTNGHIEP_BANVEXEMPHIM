  ...
  return (
    <div className="employee-self-shell">
          ...
          <div className="user-profile" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            ...
            {isMenuOpen && (
              <div className="dropdown-menu-custom">
                ...
                {activeShift && (
                  <>
                    <NavLink to="/staff/sales" className="dropdown-item-custom">
                      <ShoppingCart size={16} />
                      <span>Bán hàng POS</span>
                    </NavLink>
                    <NavLink to="/staff/dashboard" className="dropdown-item-custom">
                      <LayoutDashboard size={16} />
                      <span>Bàn làm việc</span>
                    </NavLink>
                  </>
                )}
                <NavLink to="/staff/ca-lam" className="dropdown-item-custom">
                  <Calendar size={16} />
      ...
      <main className="employee-self-main">
        {!activeShift && (
          <div className="mx-4 mt-3 alert alert-info mb-0 border-0 shadow-sm" style={{ background: "rgba(14, 165, 233, 0.1)", color: "#7dd3fc" }}>
            <i className="fas fa-info-circle me-2"></i>
            Hệ thống đang ở chế độ **Ngoài ca**. Bạn chỉ có thể xem lịch làm việc.
          </div>
        )}
        <Outlet />
      </main>
      ...
      <style>{`
        ...
        .employee-self-header {
          ...
          padding: 0 24px; border-bottom: 1px solid rgba(148,163,184,0.1);
        ...
        .employee-self-main { flex: 1; overflow: hidden; display: flex; flex-direction: column; position: relative; }
      `}</style>
    </div>
  );
}
