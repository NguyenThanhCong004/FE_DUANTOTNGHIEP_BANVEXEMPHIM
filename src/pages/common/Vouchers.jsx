...
import { Container, Row, Col, Badge, Button, Modal, Form, InputGroup } from "react-bootstrap";
...
export default function VoucherExchange() {
  ...
  const canRedeem = (v) =>
    v.status === "active" &&
    v.point_voucher > 0 &&
    userPoints >= v.point_voucher &&
    !redeemed.includes(v.Vouchers_id);
    ...
    <Layout>
      <style>{`
        ...
        .points-badge {
          ...
          align-items: center;
          gap: 12px;
          box-shadow: 0 0 32px rgba(233,30,140,0.3);
        }
        .points-badge .pts-num {
          font-family: 'Bebas Neue', sans-serif;
          font-size: 36px;
          color: var(--yellow);
          line-height: 1;
        ...
        .search-box .form-control:focus {
          ...
          background: rgba(212,226,25,0.04) !important;
        }
        ...
        .voucher-card.expired .card-stripe {
          background: rgba(255,255,255,0.2);
        }
        .voucher-card.redeemed-card .card-stripe {
          background: var(--yellow);
        }
        ...
        .btn-redeem:hover {
          box-shadow: 0 0 28px rgba(233,30,140,0.55) !important;
        ...
        .success-toast {
          ...
          font-size: 14px;
          letter-spacing: 0.5px;
          box-shadow: 0 8px 40px rgba(0,0,0,0.5), 0 0 24px rgba(212,226,25,0.2);
          z-index: 9999;
      ...
      <div className="voucher-page mt-5">
        <Container fluid="xl">
          ...
          <Row className="align-items-end mb-4 gy-3">
            <Col>
              ...
              <h1 className="page-title mb-0">
                ĐỔI <span>VOUCHER</span>
              </h1>
            </Col>
            <Col xs="auto">
              <div className="points-badge">
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.6)", textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 2 }}>
                    Điểm của bạn
                  </div>
          ...
          {/* result count */}
          <div className="result-count mb-3">
            {filtered.length} voucher{filtered.length !== 1 ? "" : ""}
          </div>
          ...
          {successCode && (
            ...
            >
              ...
              <button
                ...
                style={{ marginLeft: "auto", background: "none", border: "none", color: "rgba(212,226,25,0.5)", cursor: "pointer", fontSize: 18 }}
              >×</button>
            </div>
          )}
                ...
                return (
                  <Col key={v.Vouchers_id} xs={12} sm={6} xl={4}>
                    <div
                      ...
                      onClick={() => {
                        if (!isExpired && !isRedeemed && !isFree) {
                          setSelected(v);
                          setShowModal(true);
                        }
                      }}
                      ...
                      <div className="card-body-inner">
                        ...
                        <div className="d-flex align-items-baseline gap-2 mb-1">
                          ...
                          <span className="discount-unit">
                            {v.discount_type === "fixed" ? "GIẢM" : "GIẢM"}
                          </span>
                        </div>
                        ...
                        {/* meta info */}
                        <div className="meta-row mb-1">
                          🛒 Đơn tối thiểu: <strong>{fmt(v.min_order_value)}</strong>
                        </div>
                        <div className="meta-row mb-3">
                          📅 <strong>{fmtDate(v.start_date)}</strong> – <strong>{fmtDate(v.end_date)}</strong>
                        </div>

                        {/* bottom row: point cost + action */}
                        <div className="d-flex align-items-center justify-content-between gap-2 flex-wrap">
                          {isFree ? (
                            <span style={{ fontSize: 12, fontWeight: 700, color: "#81c784", letterSpacing: 0.5 }}>
                              🎁 Miễn phí
                            </span>
                          ) : (
                            <span className={`point-cost${!enough ? " not-enough" : ""}`}>
                              ⭐ {v.point_voucher.toLocaleString()} điểm
                            </span>
                          )}

                          {isRedeemed ? (
                            <button className="btn-redeemed" onClick={(e) => e.stopPropagation()}>
                              ✓ Đã đổi
                            </button>
                          ) : isExpired ? (
                            <button className="btn-redeem" disabled onClick={(e) => e.stopPropagation()}>
                              Hết hạn
                            </button>
                          ) : isFree ? (
                            <button
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title style={{ fontFamily: "'Bebas Neue'", fontSize: 24, letterSpacing: 3, color: "#fff" }}>
        ...
        {selected && (
          <Modal.Body style={{ padding: "24px" }}>
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 20, marginBottom: 16, border: "1px solid rgba(255,255,255,0.08)" }}>
              <div style={{ fontFamily: "'Bebas Neue'", fontSize: 32, letterSpacing: 2, background: "linear-gradient(135deg, #e91e8c, #7b1fa2)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                {selected.discount_type === "fixed"
                  ? `Giảm ${fmt(selected.value)}`
                  : `Giảm ${selected.value}%`}
              </div>
              <div style={{ fontFamily: "'Bebas Neue'", fontSize: 16, letterSpacing: 3, color: "rgba(255,255,255,0.5)", marginTop: 4 }}>
                MÃ: {selected.Code}
              </div>
            </div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", lineHeight: 2, fontWeight: 600 }}>
              <div>🛒 Đơn tối thiểu: <span style={{ color: "#fff" }}>{fmt(selected.min_order_value)}</span></div>
              <div>📅 Hết hạn: <span style={{ color: "#fff" }}>{fmtDate(selected.end_date)}</span></div>
              {selected.point_voucher > 0 && (
                <div>⭐ Chi phí: <span style={{ color: "var(--yellow)" }}>{selected.point_voucher.toLocaleString()} điểm</span></div>
              )}
            </div>
            {selected.point_voucher > 0 && (
              <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(212,226,25,0.06)", borderRadius: 10, border: "1px solid rgba(212,226,25,0.2)", fontSize: 13, color: "rgba(255,255,255,0.5)", fontWeight: 600 }}>
                Số điểm sau khi đổi:{" "}
                <span style={{ color: "var(--yellow)", fontFamily: "'Bebas Neue'", fontSize: 18, letterSpacing: 1 }}>
