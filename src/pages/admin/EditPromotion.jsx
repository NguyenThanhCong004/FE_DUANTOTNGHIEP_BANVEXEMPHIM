...
import { ArrowLeft, Film, Calendar, Percent, Info, CheckCircle2, AlertTriangle } from 'lucide-react';
...
const EditPromotion = () => {
  ...
  // Dữ liệu ban đầu giả lập dựa trên ID
  const initialData = {
    title: 'Ưu đãi phim tháng 3',
    discount_percent: '20',
    start_date: '2026-03-01',
    end_date: '2026-03-31',
    selectedMovieIds: [1, 2, 4], // Phim đang được áp dụng
    description: 'Chương trình ưu đãi đặc biệt dành cho các phim hot trong tháng.'
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (JSON.stringify(formData) === JSON.stringify(initialData)) {
      setErrors({ form: 'Bạn chưa thay đổi thông tin nào.' });
      return;
    }
  ...
  return (
    <div className="edit-promotion-page text-dark pb-5">
      <style>{`
        .promo-input {
      ...
      <div className="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="mb-0 fw-bold">Chỉnh sửa khuyến mãi #{id}</h3>
          <p className="text-muted small mb-0">Cập nhật mức giảm giá và danh sách phim áp dụng</p>
        </div>
      </div>

      {errors.form && (
        <Alert variant="warning" className="border-0 shadow-sm mb-4 d-flex align-items-center gap-2" style={{ borderRadius: '15px' }}>
                ...
                <Col md={7} className="ps-md-4 mt-4 mt-md-0">
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <div className="d-flex align-items-center gap-2 text-primary">
                      <Film size={20} />
                      <h5 className="fw-bold mb-0">Phim đang áp dụng</h5>
                    </div>
                  ...
                  <div className="mt-4 p-3 bg-light rounded-3">
                    <h6 className="fw-bold small mb-2">Xem trước danh sách áp dụng:</h6>
                    <div className="d-flex flex-wrap gap-1">
                      {formData.selectedMovieIds.length > 0 ? (
                        ...
                        <span className="text-muted small italic">Vui lòng chọn ít nhất 1 phim</span>
                      )}
                    </div>
                  </div>
